...
def process_label(intents, w2v,class_id_startpoint=0):
    ...
    for line in intents:
        ...
        # compute label vec
        label_sum = np.sum([w2v[w] for w in label if w in w2v.wv.index2word], axis = 0)
    ...
    for w in y_tr:
        ...
        for word in w.split(" "):
            if word in w2v.vocab:
                y_tr_tmp.append(w2v.vocab[word].index)
            else:
                y_tr_tmp.append(0)
        while(1):
            if len(y_tr_tmp) < 5:
...
def read_datasets_gen(dataSetting):
    ...
    print("UNSEEN_CLASS",unseen_class,unseen_class[0][0])
